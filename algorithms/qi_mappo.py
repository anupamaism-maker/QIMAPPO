...
class QIMAPPOAgent:
    ...
    def __init__(self, cfg, env):
        ...
        self.pqcs   = [PQCPolicyLayer(cfg.N_QUBITS, cfg.N_LAYERS,
                                       cfg.QUANTUM_LR, cfg.OBS_DIM)
                       for _ in range(U)]

        self.actors  = [ActorNet(cfg, pqc=self.pqcs[u]) for u in range(U)]
    ...
    def train(self):
        ...
        for ep in range(cfg.N_EPISODES):
            ...
            if (ep+1)%cfg.LOG_INTERVAL==0:
                print(f"[QI-MAPPO] Ep{ep+1:5d} R={r:8.2f} "
                      f"EE={e:.3f} DL={d:.1f}% AL={al:.4f} CL={cl:.4f}")
            if (ep+1)%cfg.SAVE_INTERVAL==0:
                self.save(f"{cfg.CHECKPOINT_DIR}/qi_mappo_ep{ep+1}.npy")
        return self.history
    ...
    def _episode(self):
        ...
        for _ in range(cfg.PPO_EPOCHS):
            a,c=self._update(A,R)
            als.append(a); cls.append(c)
    ...
    def save(self, path):
        np.save(path, {f'pqc{u}':self.pqcs[u].theta for u in range(self.cfg.N_UAVS)})

    def load(self, path):
        d=np.load(path,allow_pickle=True).item()
        for u in range(self.cfg.N_UAVS): self.pqcs[u].theta=d[f'pqc{u}']